...
```
L7  PRESENTATION    audit red + ledger grey (Open Design)
L6  ORCHESTRATION   Hermes sub-context (Kimi K2.6 / DeepSeek V3.5 / local)
L5  DOMAIN MCP      ai-incident-reporting-mcp, ai-self-audit-mcp, a2a-governance-bridge-mcp
L4  AGENT MEMORY    agentmemory (v0.9.26) — shared mode
L3  KNOWLEDGE GRAPH Cognee subgraph — scope: AI incidents, audit trails, root causes, post-mortems
L2  VERSIONED HIST  Memoria (v0.4.0) — namespace "accountabilityof"
...
```bash
git clone https://github.com/CSOAI-ORG/accountabilityof-hive
cd accountabilityof-hive
...
`https://accountabilityof.ai/.well-known/agent-card.json` — see `agent-card.json`.
